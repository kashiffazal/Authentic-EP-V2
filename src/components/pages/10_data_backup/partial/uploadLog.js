...
import { HTTP, TableColumnFilter, InsertRowInList, DeleteRowFromList, FileDownload, AccessControl } from '../../../services';
...
class UploadLog extends Component {
  ...
  render() {
    ...
    const actionCol = (a, b, className = false) => (
      <ScreenLoader inline={true} tip="Please Wait..." active={st.downloadLoader[b.id] || st.restoreLoader[b.id] || st.deleteLoader[b.id]}>
        <div className={`text-center ${className}`}>
          {AccessControl(159) &&
            <>
              <Tooltip title="Download">
                <button className="btnToLink action-btn" onClick={() => this.downloadBackup(b.id, b.folderName)}><i className="fs-18 las la-download link-color"></i></button>
              </Tooltip>
          ...
          {AccessControl(160) &&
            <>
              <Popconfirm title="Are you sure to restore this backup？" placement="top" onConfirm={() => this.backupRestoreWarning(b.id, b.folderName)} okText="Yes" cancelText="No">
                <Tooltip title="Restore">
                  <button className="btnToLink action-btn"><i className="fs-18 las la-sync link-color"></i></button>
                </Tooltip>
              </Popconfirm>
              <i className="list_view_icon_sap las la-redo"></i>
            </>
          ...
          {AccessControl(161) &&
            <Popconfirm title="Are you sure to delete this backup？" placement="topRight" onConfirm={() => this.deleteBackup(b.id, b.folderName, b.status)} okText="Yes" cancelText="No">
              <Tooltip title="Delete">
                <button className="btnToLink action-btn"><i className="fs-18 las la-trash-alt link-color"></i></button>
              </Tooltip>
            </Popconfirm>
          }
        </div>
    ...
    const columns = [
      {
        ...
        dataIndex: 'backupCreatedDate_formatted',
        width: '16%',
        align: 'center',
        sorter: (a, b) => a.backupCreatedDate.localeCompare(b.backupCreatedDate),
        ...TableColumnFilter('backupCreatedDate_formatted')
      }, {
        title: 'Uploaded By',
        dataIndex: 'name',
        width: '13%',
        ...
        width: '16%',
        align: 'center',
        sorter: (a, b) => a.inserted_date.localeCompare(b.inserted_date),
        ...TableColumnFilter('inserted_date_formatted')
      }];

    AccessControl('159,160,161') &&
      columns.push({
        title: 'Action',
        width: '10%',
        align: 'center',
        ...
        <ScreenLoader active={st.loader}>
          <DataTable
            ...
            customFilterCol={[
              ...
              { label: 'Backup Date & Time', value: 'backupCreatedDate_formatted' },
              { label: 'Uploaded By', value: 'name' },
              { label: 'Upload Date & Time', value: 'inserted_date_formatted' }
            ]}
          />
        </ScreenLoader>
  ...
  componentDidMount() { this.getData('upload'); }//End componentDidMount
...
export default connect('', StorePost)(withRouter(UploadLog));