...
class onHoldLog extends Component {
  state = {
    loader: false,
    updateStatusLoader: {},
    data: [],
  ...
  getData = () => {
    ...
    HTTP('get', '/shiftReplacementRequest/get/getList/hold').then(res => {
  ...
  changeStatus = (newStatus, row) => {
    ...
    HTTP('post', '/shiftReplacementRequest/post/changeStatus/', postData).then(res => {
      ...
      if (newStatus === 'requested') { this.props.onRequested(row); }
    ...
    const actionCol = (record, row, className = false) => (
      <ScreenLoader active={this.state.updateStatusLoader[row.id]} inline={true} tip="Please wait...">
        <div className={`text-center ${className}`}>
          {window.webviewMobile && AccessControl(113) &&
            <>
          ...
          {AccessControl(116) &&
            <>
              <Tooltip placement="top" title="Requested">
                <Popconfirm title={<span>Are you sure to change status as <strong>Requested</strong>?</span>} onConfirm={() => this.changeStatus('requested', row)} okText="Yes" cancelText="No">
                  <button className="btnToLink"><i className="fs-18 las la-exclamation-circle link-color" /></button>
                </Popconfirm>
              </Tooltip>
              <i className="list_view_icon_sap las la-redo"></i>
