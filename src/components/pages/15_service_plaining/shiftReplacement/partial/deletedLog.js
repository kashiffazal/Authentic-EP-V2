...
class DeletedLog extends Component {
  state = {
    loader: false,
    updateStatusLoader: {},
    data: [],
  ...
  getData = () => {
    ...
    HTTP('get', '/shiftReplacementRequest/get/getList/deleted').then(res => {
    ...
    const actionCol = (record, row, className = false) => (
      <ScreenLoader active={this.state.updateStatusLoader[row.id]} inline={true} tip="Please wait...">
        <div className={`text-center ${className}`}>
          {window.webviewMobile && AccessControl(118) &&
            <>
