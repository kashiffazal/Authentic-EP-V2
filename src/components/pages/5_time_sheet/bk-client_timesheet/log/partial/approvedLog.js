...
class ClientTimesheetApproved extends Component {
  state = {
    getLoader: false,
      ...
      columns.push({
        ...
        render: (record, row) =>
          <div className="text-center">
            {AccessControl(33) && <button title="View Details" className="btnToLink" onClick={() => this.setState({ visibleViewModal: true, pdf_path: row.pdf_path })}><i className="fs-18 las la-table link-color"></i></button>}
            {AccessControl(33) && <i className="list_view_icon_sap las la-redo"></i>}
            {AccessControl(32) && <button title="Edit" className="btnToLink" onClick={() => SaveArrLocalStorage(row.id, "clientTimesheet")}><i className="fs-18 las la-edit link-color"></i></button>}
          </div>
      });
    return (
      <div>
        ...
        <ViewPDFInModal title='View Client Timesheet Detailed File' show={st.visibleViewModal} close={() => this.setState({ visibleViewModal: false })} pdfPath={st.pdf_path} />
      </div>
    )//End Return statement
  }//End End Render
  componentDidMount() {
    this.setState({ getLoader: true });
    HTTP('get', '/timesheetClient/get/getApprovedList/').then(res => {
...
export default ClientTimesheetApproved;
