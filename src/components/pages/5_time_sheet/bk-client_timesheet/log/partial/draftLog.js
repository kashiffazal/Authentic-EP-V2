...
class ClientTimesheetDraft extends Component {
  state = {
    getLoader: false,
    list_data: []
  };
    ...
    const columns = [
      {
        ...
        width: '5%',
        render: (record, row) =>
          <button title="Edit" className="btnToLink" onClick={() => SaveArrLocalStorage(row.id, "clientTimesheet")}><i className="fs-18 las la-edit link-color"></i></button>
      }
    ];
    return (
      <div>
  ...
  componentDidMount() {
    ...
    HTTP('get', '/timesheetClient/get/getDraftList/').then(res => {
...
export default ClientTimesheetDraft;
