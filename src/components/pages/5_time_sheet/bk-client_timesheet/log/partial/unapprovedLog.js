...
class ClientTimeSheetUnapproved extends Component {
  state = {
    getLoader: false,
    list_data: []
  };
  ...
  render() {
    ...
    AccessControl(30) &&
      columns.push({
        ...
        width: '5%',
        render: (record, row) =>
          AccessControl(30) && <button title="Edit" className="btnToLink" onClick={() => SaveArrLocalStorage(row.id, "clientTimesheet")}><i className="fs-18 las la-edit link-color"></i></button>
      })
  ...
  componentDidMount() {
    ...
    HTTP('get', '/timesheetClient/get/getUnapprovedList/').then(res => {
...
export default ClientTimeSheetUnapproved;
