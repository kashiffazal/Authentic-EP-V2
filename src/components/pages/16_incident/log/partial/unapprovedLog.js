...
import { Tooltip, Button, Popconfirm, Popover } from 'antd';
...
class IncidentListUnapproved extends Component {
  state = {
    getLoader: false,
    deleteLoader: {},
    list_data: [],
          ...
          {!pr.isSPW &&
            <React.Fragment>
              ...
              {AccessControl(57) &&
                <>
                  <i className="list_view_icon_sap las la-redo" />
                  <Button type="primary" ghost size="small" className="btn-approve" onClick={() => SaveArrLocalStorage({ id: row.id, officeSection: true }, "incidentForm")}>Approve</Button>
                </>
              }
            </React.Fragment>
          }
  ...
  componentDidMount() {
    ...
    HTTP('get', '/incident/get/getList/unapprove').then(res => {
...
export default IncidentListUnapproved;
