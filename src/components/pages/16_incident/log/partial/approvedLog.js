...
class IncidentListApproved extends Component {
  state = {
    getLoader: false,
    deleteLoader: {},
    list_data: [],
              ...
              {AccessControl(61) &&
                <>
                  ...
                  <Tooltip placement="top" title='Edit' mouseEnterDelay={0.5}>
                    <button className="btnToLink" onClick={() => SaveArrLocalStorage({ id: row.id, officeSection: true }, "incidentForm")}><i className="fs-18 las la-edit link-color"></i></button>
                  </Tooltip>
                </>
              }
              {AccessControl(62) &&
                <>
                  <i className="list_view_icon_sap las la-redo" />
                  <Tooltip placement="bottom" title='Deleted' mouseEnterDelay={0.5}>
                    <Popconfirm
  ...
  componentDidMount() {
    ...
    HTTP('get', '/incident/get/getList/approved').then(res => {
...
export default IncidentListApproved;
