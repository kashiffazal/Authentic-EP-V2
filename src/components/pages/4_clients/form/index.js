...
import ClientFormExternal from './mainForm';
import { HTTP } from '../../../services';

class ClientFormInternal extends Component {
  state = {
    getLoader: false,
    recentData: [],
    ...
    return (
      <React.Fragment>
        <PageTitle
          ...
          titleSpan="Participant Referral"
          titleHeading="Form"
          ...
          breadcrumbWithRender={
            <div className="renderContentWithBreadcrumb">
              <Button size="small" ghost type="primary" className="m-b-0" onClick={() => window.open(process.env.PUBLIC_URL + '/#/externalWebPages/clientReferralForm', "_blank")}>External Form</Button>
            </div>
          }
        />
                        ...
                        return (
                          <div key={item.key} className="widget-list">
                            <p>
                              ...
                              <span>{item.status} <i className="las la-arrow-right" /> <button className="btnToLink link-color" onClick={() => this.setState({ visibleViewModal: true, pdf_path: item.pdf_path, clientName: item.name })}>View Details</button></span>
                            </p>
                          </div>
                        )
                      })
  ...
  componentDidMount() {
    if (!window.webviewMobile) {
      ...
      HTTP('get', '/client/get/getRecentClient/').then(res => {
...
export default ClientFormInternal;